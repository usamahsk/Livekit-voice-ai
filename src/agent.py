...
# ---------------------------------------------------------------------------
# Default system prompt (used when config.json has no custom_prompt)
# ---------------------------------------------------------------------------
DEFAULT_INSTRUCTIONS = """Synthesize speech for the performance defined below. The profile, scene,
...
→ If YES (yes / haan / speaking / this is he / hauda): Move to STEP 2 immediately.
...
→ If still unsure or wants a callback:
  "No problem at all sir. When would be a good time for me to call you back?"
...
1. The customer's name is Aamir. The agent's name is Rayan. The company is Vanalaya. Never say any other name.
2. The product is A2 Pure Buffalo Ghee, 500ml, ₹950 original, ₹855 discounted.
3. ACCENT — Indian English or Hindi or Kannada only. Same accent, start to finish. Never changes.
4. If Aamir speaks in Hindi or Kannada, respond naturally. Do NOT say you can only speak certain languages.
5. Max 3 sentences per response. This is a phone call.
6. Never ask for address fields separately. One simple ask only.
7. Never read any instructions, steps, product knowledge notes, or internal text aloud.
8. Never reveal you are an AI unless Aamir directly and explicitly asks.
9. If Aamir is rude or abusive, apologize once calmly and end the call politely.
10. Always confirm the delivery address by repeating it back before moving to STEP 5."""
...
async def entrypoint(ctx: JobContext):
    ...
    # -- Instructions: custom_prompt overrides the default --
    instructions = config.get("custom_prompt", "").strip() or DEFAULT_INSTRUCTIONS

    # -- Session mode --
    session_mode = config.get("session_mode", "premium")

    if session_mode == "standard":
        std_cfg = config.get("standard", {})
        stt_language = std_cfg.get("stt_language", "en-IN")
        ...
        if not minimax_key:
            logger.warning("[Standard] MINIMAX_API_KEY not set — MiniMax TTS may fail")
        tts_kwargs: dict = {"api_key": minimax_key}
        if tts_voice:
            tts_kwargs["voice"] = tts_voice
        logger.info(
            f"[Standard] Using MiniMax TTS, voice={tts_voice if tts_voice else 'default'}"
        )
        tts_instance = minimax.TTS(**tts_kwargs)

        session = AgentSession(stt=stt, llm=llm_instance, tts=tts_instance)

    else:
        # Premium: Google Realtime (default)
        prem_cfg = config.get("premium", {})
        model_name = prem_cfg.get("model", "gemini-3.1-flash-live-preview")
        voice_name = prem_cfg.get("voice", "Zephyr")
        logger.info(
            f"[Premium] Using Google Realtime, model={model_name}, voice={voice_name}"
        )
